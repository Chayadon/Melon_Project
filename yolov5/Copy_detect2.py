...
msg_img = []
token = 'lu8NMynNyKixr29iyN9KMLfbsrlAqwIEEqqV2Hsq236'
is_BB_empty = True
name_of_pic = ""
name_of_BB = ""
name_of_confi = ""
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            seen += 1
            is_BB_empty = True
            parent_dir = "/home/leaf/Desktop/yolov5/Summary/" #Edit Here
            ...
            name_of_BB = "BB_" + file_name_collector(p.name) + ".txt"
            name_of_confi = file_name_collector(p.name) + ".txt"
            ...
            Label_Collector(str_labels, file_name_collector(str(p))) # Get path of pic
            str_labels.clear()
            if view_img:
                if p not in windows:
                    windows.append(p)
            ...
            if save_img:
                ...
                shutil.move(parent_dir+name_of_pic, path_dir )
                shutil.move(parent_dir+name_of_BB, path_dir )
                shutil.move(parent_dir+name_of_confi, path_dir )
...
def LINENotification(imageName,txt):
  ...
  , params = payload
  , files = {'imageFile': open(imageName, 'rb') })

def ResizeImage(imageName,p):
    path_to_save = "/home/leaf/Desktop/yolov5/Summary/" # images are saved #Edit Here
...
def Label_Collector(txt,namefile):
  path_of_file = "/home/leaf/Desktop/yolov5/Summary/" + namefile + ".txt" #Edit Here
...
def BoundingBox(xy,NameOfFile):
  #print(NameOfFile)
  path_of_file = "/home/leaf/Desktop/yolov5/Summary/" + "BB_" + NameOfFile + ".txt" #Edit Here
  if( xy == 0 ):
    file = open(path_of_file,'a+') #1
    file.write("None")
  else:
    #print("Size of xy",len(xy))
    file = open(path_of_file,'a+') #2
    x_y = find_xy(str(xy))
    for i in x_y:
        file.write(str(i) + " ")
    file.write("\n")
